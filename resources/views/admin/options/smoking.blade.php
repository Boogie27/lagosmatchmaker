                    ...
                    <div class="col-md-12">
                        <nav aria-label="breadcrumb" class="float-right mt-1">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">Smoking</li>
                            </ol>
                        </nav>
                        <h4 class="mb-1 mt-0">Smoking</h4>
                        ...
                        <div class="card">
                            <div class="card-body">
                               ...
                               <div class="table-top pb-2">
                                    <a href="#" id="add_content_btn" class="mini-btn">Add Smoking</a>
                               </div>
                               <div class="table-responsive"> <!-- table start-->
                                    <table id="datatable-buttons" class="table table-striped dt-responsive nowrap">
                                        ...
                                        <tbody id="parent_table">
                                            ...
                                            <tr>
                                                <td>
                                                    <div class="genotype-name">{{ ucfirst($smoke->title) }}</div>
                                                </td>
                                                <td>
                                                    <div class="check-box {{ $smoke->is_featured ? 'active' : ''}}">
                                                        <a href="#" data-name="{{ $smoke->title }}" id="{{ $smoke->id }}" class="feature-confirm-box-open"></a>
                                                    </div>
                                                </td>
                                                <td>{{ date('d M Y', strtotime($smoke->date)) }}</td>
                                                <td>
                                                    <div class="drop-down">
                                                        <i class="fa fa-ellipsis-h drop-down-open"></i>
                                                        <ul class="drop-down-body">
                                                            <li>
                                                                <a href="#" data-name="{{ $smoke->title }}" id="{{ $smoke->id }}" class="content-edit-btn">Edit</a>
                                                            </li>
                                                            <li>
                                                                <a href="#" data-name="{{ $smoke->title }}" id="{{ $smoke->id }}" class="content-delete-btn">Delete</a>
                                                            </li>
                                                        </ul>
                                                    </div>
                                                </td>
                                ...
                                <div id="bottom_table_part">
                                    @if(!count($smoking))
                                    <div class="text-center">There are no smoking options yet!</div>
                                    @endif
                                    @if(count($smoking))
                                    <div class="paginate">{{ $smoking->links("pagination::bootstrap-4") }}</div>
                                    @endif
                                </div>
                            </div> <!-- end card body-->
                        </div> <!-- end card -->
                   ...
                   <form action="" method="POST">
                        <div class="col-lg-12">
                            <div class="form-group">
                                <label for="userName">Smoking<span>*</span></label>
                                <input type="text" parsley-trigger="change" id="form_content_input" placeholder="Enter smoking" class="form-control" value="">
                ...
                <div class="text-left">
                   <form action="" method="POST">
                        <div class="col-lg-12">
                            <div class="form-group">
                                <label for="userName">Smoking<span>*</span></label>
                                <input type="text" parsley-trigger="change" id="add_form_content_input" placeholder="Enter smoking" class="form-control" value="">
                                <div class="alert-form alert_0 text-danger"></div>
                            </div>
                        </div>
                        <div class="col-lg-12">
...
function table_check(){
    ...
    if(table.length == 0){
        $("#bottom_table_part").html("<div class='text-center'>There are no smoking options yet!</div>")
    }
}
...
function edit_marital_status(){
    ...
    $.ajax({
        url: "{{ url('/admin/ajax-edit-smoking') }}",
        method: "post",
        data: {
            id: id,
            smoking: smoking,
        ...
        success: function (response){
            if(response.error){
                $(".alert_0").html(response.error.smoking)
            }else if(response.data){
                $(content_parent).html(smoking)
                $(".modal-alert-popup").hide()
                $(dataName).attr('data-name', smoking)
                bottom_alert_success('Smoking option has been updated!')
            }else{
                $(".modal-alert-popup").hide()
...
function validate_form(smoking){
    var state = false;
    if(smoking == ''){
        state = true;
        $(".alert_0").html('*Smoking field is required')
    }else if(smoking.length > 50){
        state = true;
...
$(".content-delete-btn").click(function(e){
    ...
    apend_message("<p>Do you wish to delete smoking option <br><b>"+name+"</b></p>")
})
...
$("#delete_confirm_submit_btn").click(function(e){
    ...
    $.ajax({
        url: "{{ url('/admin/ajax-delete-smoking') }}",
        method: "post",
        data: {
            id: id,
        },
        success: function (response){
            if(response.data){
                ...
                $(".modal-alert-popup").hide()
                bottom_alert_success('Smoking has been deleted!')
                table_check()
            }else{
                $(".modal-alert-popup").hide()
                bottom_alert_error('Network error, try again later!')
...
function add_content(){
    $(".alert-form").html('')
    var smoking = $("#add_form_content_input").val()
    ...
    $.ajax({
        url: "{{ url('/admin/ajax-add-smoking') }}",
        method: "post",
        data: {
            smoking: smoking,
        },
...
$(".feature-confirm-box-open").click(function(e){
    ...
    if($(featured).hasClass('active')){
        apend_message("<p>Do you wish to unfeature the smoking option <br><b>"+content+"</b></p>")  
    }else{
        apend_message("<p>Do you wish to feature the smoking option <br><b>"+content+"</b></p>") 
    }
})
...
$("#feature_confirm_submit_btn").click(function(e){
    ...
    $.ajax({
        url: "{{ url('/admin/ajax-feature-smoking') }}",
        method: "post",
        data: {
            id: id,
        },
        success: function (response){
            if(response.featured){
                ...
                bottom_alert_success('Smoking option has been featured!')
            }else if(response.unfeatured){
                $(featured).removeClass('active')
                bottom_alert_success('Smoking option has been unfeatured!')
            }else{
                bottom_alert_error('Network error, try again later!')
            }
            $(".modal-alert-popup").hide()
